...
export function LessonArticle({lesson,onAsk}:{lesson:ExplainResponse;onAsk:(question:string)=>void}){
 ...
 return <article id="lesson-reading" className={styles.article} aria-labelledby="lesson-title">
  ...
  <div className={styles.readingBody}>
   ...
   {lesson.video_lesson_script||lesson.media_ready_content?<div className={styles.sceneNotice}><StatusPanel tone="info" title="Scene-ready lesson structure" message="This lesson includes scene, narration, and visual-cue material. It is not an encoded video; use the Media panel to prepare a downloadable ZIP package."/></div>:null}
   {lesson.practice_questions.length?<section id="lesson-check" className={styles.lessonSection} aria-labelledby="lesson-check-title" tabIndex={-1}><h3 id="lesson-check-title">Check your understanding</h3><p className={styles.practiceIntro}>Choose a question to move it into the Ask workspace.</p><div className={`${styles.questionList} ${styles.screenQuestions}`}>{lesson.practice_questions.slice(0,5).map((question,index)=><button key={`${normalizeLessonText(question)}-${index}`} className={styles.question} type="button" onClick={()=>onAsk(question)} lang={devanagariPattern.test(question)?"hi":undefined}>{question}</button>)}</div><ol className={styles.printQuestions}>{lesson.practice_questions.slice(0,5).map((question,index)=><li key={`${normalizeLessonText(question)}-print-${index}`} lang={devanagariPattern.test(question)?"hi":undefined}>{question}</li>)}</ol></section>:null}
  </div>
 </article>
}
