...
Launch uses Brevo's transactional HTTPS API: `EMAIL_DELIVERY_MODE=email`, `EMAIL_TRANSPORT=brevo`, `BREVO_BASE_URL=https://api.brevo.com/v3`, and a backend-only `BREVO_API_KEY`. Render Free blocks outbound SMTP ports 25, 465, and 587, which is why locally working Gmail SMTP cannot be the Render transport. Brevo's free quota is currently 300 sends per day, but quotas and provider terms can change and must be rechecked operationally.

An individually verified Gmail sender is acceptable for the initial beta, but it is a compromise: free-address senders can be rewritten by Brevo to a provider-controlled transactional domain, recipients may see a less recognizable address, and deliverability is weaker than an authenticated owned domain. The Vercel-owned `adhyantra-ai.vercel.app` hostname is not an owned email domain and must not be presented as one. After acquiring a custom domain, authenticate it in Brevo with the provider's reviewed DNS records, create a sender on that domain, update `EMAIL_FROM_ADDRESS`, and reverify delivery before cutover. SMTP and Resend remain supported for non-Render deployments; neither is an automatic fallback from Brevo.

Gemini TTS requires `TTS_PROVIDER=gemini`, `GEMINI_API_KEY`, `TTS_GEMINI_MODEL`, and `TTS_GEMINI_VOICE`. The independently verified text route is `AI_PROVIDER=gemini`, `AI_PROVIDER_CHAIN=gemini`, and `GEMINI_MODEL=gemini-3.6-flash`. With `ALLOW_MOCK_AI_IN_PRODUCTION=false`, exhaustion returns service unavailable instead of mock content. A controlled check on 2026-08-27 found that the advertised `gemini-2.5-flash` text model returned a provider-side 404 for this project; the separate Gemini 2.5 Flash TTS model succeeded and remains configured. The single authorized Groq launch check also returned a provider-side 404, so `render.yaml` does not request a Groq credential or retain Groq in the launch chain. `TTS_TIMEOUT_SECONDS`, `TTS_MAX_INPUT_CHARACTERS`, and `TTS_MAX_SEGMENTS` bound each request and job. Payments remain intentionally disabled with `PAYMENT_PROVIDER=disabled`.
