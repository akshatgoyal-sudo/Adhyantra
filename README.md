...
- The repository contains provider-neutral staging scripts, a runbook, and a tracked `render.yaml` for the one-service free-tier topology. This configuration is not evidence that Render or Vercel is deployed or healthy.
- Alembic owns PostgreSQL schema changes. API and worker startup validate the database revision and required tables without DDL; only explicit development/test SQLite startup retains automatic table creation and compatibility updates. See `docs/migrations.md`.
...
- From one explicitly authorized migration owner, migrate before first boot or after schema-related changes:
...
- PostgreSQL has a reviewed Alembic model/schema baseline. Runtime startup never creates or alters PostgreSQL schema; it fails closed unless the database is at the single source head and all required tables exist. Explicit development/test SQLite startup still creates missing tables and applies compatibility updates.
...
npm run db:migrate  # requires an explicit EXAM_GURU_MIGRATION_DB_URL
...
In local development and tests, explicitly selecting `AI_PROVIDER=mock` or including `mock` in `AI_PROVIDER_CHAIN` enables built-in mock logic:
...
This keeps local development usable offline once dependencies are installed. Deployed production with `ALLOW_MOCK_AI_IN_PRODUCTION=false` rejects a configured `mock` provider and returns a retryable service-unavailable response when Gemini and Groq are exhausted; it never fabricates production content.
...
- In production with mock disabled, provider failure or invalid JSON returns a retryable service-unavailable response. Explicit development/test mock configurations retain deterministic fallback responses.
...
- `backend/services/ai_service.py` owns prompt construction, provider selection, response validation, strict production failure behavior, and development/test mock fallback.
...
- If a live provider call fails or returns incomplete JSON, the backend tries the next configured live provider. Strict production returns a sanitized retryable error after exhaustion; explicit development/test mock mode may return the deterministic mock response shape.
